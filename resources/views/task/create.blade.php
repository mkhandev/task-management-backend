    ...
    <form action="{{ route('tasks.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description"
                class="border rounded w-full p-2 focus:outline-none @error('description') border-red-500 @enderror">{{ old('description') }}</textarea>
            @error('description')
                <div class="mt-1 text-sm text-red-500">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <select name="status" id="status"
                ...
                <option value="Work In Progress" {{ old('status') === 'Work In Progress' ? 'selected' : '' }}>In Progress
                </option>
